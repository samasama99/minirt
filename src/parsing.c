...
/*   Updated: 2022/07/28 14:39:05 by orahmoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_optional_double parse_ratio(const char *target)
{
  ...
  if (fractional.error)
    return error;
  if (fractional.value / 10. + whole.value > 1.0 || fractional.value / 10. + whole.value < 0.0)
    return error;
  return (t_optional_double){
...
t_optional_point parse_position(const char *target)
{
  ...
  if (array.error || count_char(target, ',') != 2 || array_len(array.value) != 3)
