...
int draw_in_canvas(t_image canvas, t_world w) {
  for (int y = 0;y < canvas_pixel_y - 1;++y) {
    ...
    for (int x = 0;x < canvas_pixel_x - 1;++x) {
      ...
      const t_pos position = point(world_x, world_y, wall_z);
      const t_ray r = {ray_origin, vec_normalize(vec_sub(position, ray_origin), sqrt)};
      const t_rgb c = color_at(w, r);
      put_pixel_to_image(canvas, point(x, y, 0), c);
    }
  }
  clear_window();
