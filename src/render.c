...
static t_thread_data	create_thread_data(int y, t_camera c, t_world w,
	t_image canvas)
{
...
void	render(t_camera c, t_world w)
{
	...
	id = ft_malloc(sizeof(t_thread_data) * (int)c.vsize - 1);
	data = ft_malloc(sizeof(t_thread_data) * (int)c.vsize - 1);
	y = 0;
	while (y < c.vsize - 1)
	{
