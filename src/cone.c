...
/*   Updated: 2022/07/26 18:59:53 by zsarir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_hit	intersect_cone(const t_cone co, const t_ray r)
{
	...
	const double	a = pow(r.direction.x, 2) + pow(r.direction.z, 2) - pow(k, 2) * pow(r.direction.y, 2);
	const double	b = 2 * (r.origin.x * r.direction.x + r.origin.z * r.direction.z - r.origin.y * r.direction.y * pow(k, 2)
						- r.direction.x * co.center.x - r.direction.z * co.center.z + co.height * pow(k, 2) * r.direction.y);
	const double	c = pow(r.origin.x, 2) + pow(r.origin.z, 2) - pow(r.origin.y, 2) * pow(k, 2)
					- 2 * (r.origin.x * co.center.x + r.origin.z * co.center.z) 
					- pow(co.height, 2) * pow(k, 2) + 2 * co.height * pow(k, 2) * r.origin.y;
	const double	discriminant = b * b - 4 * a * c;

	double hi = co.center.y + co.height;
