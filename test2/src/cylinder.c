...
t_hit	intersect_cylinder(const t_cylinder cy, const t_ray r)
{
	...
	// printf ("dis = %f sqrt(dis) = %f a = %f b = %f c = %f\n", discriminant, sqrt(discriminant), a, b, c);
...
{
  // return s.normal;
//    if (is_equal_double(local_point.x, 0) == false && is_equal_double(local_point.z, 0) == false)
//      return normalize(vector(1, 0, 0));
